...
class VietnameseCalendarApp:
    def __init__(self, root):
        ...
        # Đăng ký xử lý sự kiện tắt máy
        self.register_shutdown_handler()

    def register_shutdown_handler(self):
        """Đăng ký xử lý sự kiện tắt máy Windows"""
        ...
        # Tạo cửa sổ ẩn để nhận thông báo
        wc = win32gui.WNDCLASS()
        wc.lpfnWndProc = wndProc
        wc.lpszClassName = "LichAmDuongShutdownHandler"
        win32gui.RegisterClass(wc)
        hwnd = win32gui.CreateWindow(wc.lpszClassName, 
                                   "LichAmDuongShutdownHandler",
                                   win32con.WS_OVERLAPPED,
                                   0, 0, 0, 0, 0, 0, 0, None)
        
        # Lưu handle để tránh bị thu hồi bộ nhớ
        self.shutdown_hwnd = hwnd

    def clean_exit(self):
        """Thoát ứng dụng một cách sạch sẽ"""
        try:
            # Dừng system tray icon
            ...
            if hasattr(self, 'shutdown_hwnd'):
                win32gui.DestroyWindow(self.shutdown_hwnd)
    ...
    def create_calendar_grid(self):
        ...
        for col, day in enumerate(weekdays):
            header_cell = tk.Frame(self.grid_frame, width=40, height=20, bd=1, relief="groove", bg="#f0f0f0")
    ...
    def create_header_cell(self, col):
        ...
        cell_frame = tk.Frame(
            self.grid_frame,
            width=27,
            height=20,  # Giảm chiều cao header
            bd=1,
            relief="groove",
            bg="#f0f0f0"
        )
